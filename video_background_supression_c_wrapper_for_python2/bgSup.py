...
@ffi.callback("char *(char*, int, int)")
def startStream(buffer, width, height):

	#convert c buffer to bytes
	data = ffi.buffer(buffer, ((width * height) * 3//2))[:]
	...
	#ret = ffi.from_buffer(ret)
	result = ffi.new('char []', ret.tobytes()) 
	return result
"""
#ORIGINAL CODE 
